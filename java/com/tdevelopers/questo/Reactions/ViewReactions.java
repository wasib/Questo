...
public class ViewReactions extends Fragment implements OnChartValueSelectedListener {
    ...
    public void onViewCreated(final View view, @Nullable Bundle savedInstanceState) {
        ...
        if (id != null && id.trim().length() != 0) {
            final DatabaseReference ref = FirebaseDatabase.getInstance().getReference("Question").child(id + "");

            ref.child("correct_count").addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(DataSnapshot dataSnapshot) {

                    if (dataSnapshot != null && dataSnapshot.getValue() != null)
                        correct = (Long) dataSnapshot.getValue();
                    ref.child("wrong_count").addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(DataSnapshot dataSnapshot) {

                            if (dataSnapshot != null && dataSnapshot.getValue() != null)
                                wrong = (Long) dataSnapshot.getValue();
